...
const Home = () => {
  ...
  // Fetch initial movies (Now Playing)
  useEffect(() => {
    const fetchNowPlaying = async () => {
      try {
        setLoading(true);
        const response = await axios.get(
          `${API_URL}/movie/now_playing?api_key=${API_KEY}&page=${currentPage}&language=${language}`
        );
  ...
  const handleSearchSubmit = (e) => {
    ...
    navigate(`/search?query=${encodeURIComponent(searchTerm)}&page=1&type=movie`);
  };
